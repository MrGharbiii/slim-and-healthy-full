...
import { OnboardingAPI, AnalysisAPI } from './services/apiService';
...
export function MainApp() {
  ...
  // State to track if plan has been assigned (created and ready to display)
  const [assignedPlan, setAssignedPlan] = useState(false);

  // State to store the user's plan data
  ...
  const handleAnalysisComplete = () => {
    ...
    // Check if user has a plan after analysis is completed
    checkUserPlan();
  };

  // Function to check if user has a plan
  const checkUserPlan = async () => {
    console.log('🔍 Checking if user has an existing plan...');
    try {
      const response = await AnalysisAPI.getUserPlan();
      if (response.success && response.data && response.data.data) {
        console.log('✅ Plan data retrieved successfully');

        const { requestedPlan, assignedPlan, plan } = response.data.data;
        console.log('📊 PLAN STATUS VALUES:');
        console.log('  • requestedPlan:', requestedPlan);
        console.log('  • assignedPlan:', assignedPlan);
        console.log('  • plan data exists:', !!plan);

        // Update state based on API response
        setIsPlanRequested(requestedPlan);
        setAssignedPlan(assignedPlan);

        // Always set planData, even if it's null
        setPlanData(plan);

        if (plan) {
          console.log('📋 Plan data structure:', Object.keys(plan));
        } else {
          console.log('📋 No plan data in the response');
        }
      } else {
        console.warn('⚠️ No plan data available or request failed');
  ...
  const handlePlanRequested = async () => {
    console.log(
      '🚀 Plan requested! Setting plan request and assigned states to true immediately'
    );
    // Immediately set both states to true to show the plan right away
    setIsPlanRequested(true);
    setAssignedPlan(true);

    try {
      // Make the actual API request to request a plan
      const response = await AnalysisAPI.requestActionPlan();

      if (response.success) {
        console.log('✅ Plan request submitted successfully');

        // We're already showing the plan, but still check for actual plan data in background
        const checkInterval = setInterval(async () => {
          const planResponse = await AnalysisAPI.getUserPlan();
          if (
            planResponse.success &&
            planResponse.data &&
            planResponse.data.data &&
            planResponse.data.data.assignedPlan &&
            planResponse.data.data.plan
          ) {
            console.log('✅ Plan data has been received from server!');
            setPlanData(planResponse.data.data.plan);
            clearInterval(checkInterval);
          }
        }, 5000); // Check every 5 seconds

        // Stop checking after 30 seconds regardless of result
        setTimeout(() => {
          clearInterval(checkInterval);
        }, 30000);
      } else {
        console.error('❌ Failed to submit plan request');
      }
    } catch (error) {
  ...
  // Check for existing plan when user reaches dashboard
  useEffect(() => {
  ...
  // Log plan status variables when entering plan screen
  useEffect(() => {
    if (currentScreen === 'plan') {
      console.log('🎯 USER ENTERED PLAN SCREEN');
      console.log('=======================================');
      console.log('📌 CURRENT PLAN STATUS VARIABLES:');
      console.log('  • isPlanRequested:', isPlanRequested);
      console.log('  • assignedPlan:', assignedPlan);
      console.log('  • planData exists:', !!planData);
      ...
      // Check for latest plan data when entering plan screen
      checkUserPlan();
    }
  }, [
    currentScreen,
