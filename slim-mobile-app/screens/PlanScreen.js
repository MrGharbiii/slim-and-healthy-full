...
const PlanScreen = ({
  ...
  // Log plan status values when entering the screen or when values change
  useEffect(() => {
    console.log('=========== PLAN SCREEN STATUS ===========');
    console.log('📋 isPlanRequested:', isPlanRequested);
    console.log('✅ assignedPlan:', assignedPlan);
    console.log('📊 isAnalysisCompleted:', isAnalysisCompleted);
    console.log('🗂️ planData available:', planData ? 'YES' : 'NO');

    if (planData) {
      console.log('📑 planData structure:', Object.keys(planData));
    } else {
      console.log('📑 No planData yet, but we will show the plan UI anyway');
    }

    // Log the rendering decision we would make
    if (isPlanRequested || assignedPlan) {
      console.log('🎯 DECISION: Will show plan interface');
    } else if (!isAnalysisCompleted) {
      console.log('🎯 DECISION: Will show analysis completion prompt');
    } else {
      console.log('🎯 DECISION: Will show action plan request');
    }

    console.log('========================================');
  }, [isPlanRequested, assignedPlan, isAnalysisCompleted, planData]);
  ...
  // Sample plan data for 14 days (fallback if API data is not available)
  ...
  const handleActionPlanRequest = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Notify App.js that plan was requested - this will set both isPlanRequested and assignedPlan to true
        if (onPlanRequested) {
          onPlanRequested();
        }

        // No need to navigate away or show alerts - the UI will update automatically
      } else {
  ...
  // Render API Plan Data
  const renderApiPlanData = () => {
    console.log(
      '📋 renderApiPlanData called - planData:',
      planData ? 'present' : 'absent'
    );

    // Even if planData is null/empty, we'll still show the plan UI skeleton
    const creationDate = planData?.assignedAt
      ? new Date(planData.assignedAt).toLocaleDateString()
      : new Date().toLocaleDateString();

    return (
      <View style={styles.apiPlanContainer}>
        <Animatable.View animation="fadeIn" style={styles.apiPlanSection}>
          <Text style={styles.apiPlanTitle}>Your Personalized Health Plan</Text>
          <Text style={styles.apiPlanDate}>Created on: {creationDate}</Text>

          {/* Profiles section */}
          <View style={styles.profilesSection}>
            <Text style={styles.sectionTitle}>Your Health Profiles</Text>
            {planData?.profiles && planData.profiles.length > 0 ? (
              planData.profiles.map((profile, index) => (
              ...
              <Text style={{ padding: 10, color: '#666', fontStyle: 'italic' }}>
                Your health profiles will appear here once available.
              </Text>
            )}
          </View>

          {/* Plan sections */}
          <View style={styles.planSectionsContainer}>
            <Text style={styles.sectionTitle}>Recommended Actions</Text>
            {planData?.sections && Object.keys(planData.sections).length > 0 ? (
              Object.entries(planData.sections).map((sectionEntry, index) => {
                const [sectionName, items] = sectionEntry;
                return (
                  <View key={index} style={styles.planSection}>
                    <View style={styles.sectionHeader}>
                      {sectionName === 'dietetique' && (
                        <Ionicons name="nutrition" size={24} color="#4CAF50" />
                      )}
                      {sectionName === 'activitePhysique' && (
                        <FontAwesome5
                          name="running"
                          size={24}
                          color="#2196F3"
                    ...
                    {/* Some placeholder content for recommendations */}
              ...
              <Text style={{ padding: 10, color: '#666', fontStyle: 'italic' }}>
                Your action recommendations will appear here once available.
              </Text>
            )}
          </View>

          {/* Empty state message if no data */}
          {(!planData || (!planData.profiles && !planData.sections)) && (
            <View
              style={{
                marginTop: 20,
                padding: 15,
            ...
            >
              ...
              >
                Your personalized health plan is being prepared. Please check
                back soon or pull down to refresh.
              </Text>
            </View>
          )}
        </Animatable.View>
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {/* Enhanced logging of plan status right before rendering decision */}
        {console.log(
          '🔍 RENDER DECISION - isPlanRequested:',
          isPlanRequested,
          'assignedPlan:',
          assignedPlan,
          'planData:',
        ...
        {assignedPlan || isPlanRequested
          ? // If assignedPlan is true OR isPlanRequested is true, always show the plan unconditionally
            renderApiPlanData()
          : !isAnalysisCompleted
